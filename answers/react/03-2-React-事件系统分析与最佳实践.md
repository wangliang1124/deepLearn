...
> 原文：<https://zhuanlan.zhihu.com/p/27132447>
...
## 1 引子
  ...
  * 感谢 [@颜什么都不记得适](<https://www.zhihu.com/people/21c6c06317eba1077caa412960aaf18a>) 提醒了我 key 不该用 index，否则数组中间插入一个值以后，后面就无法保持原来的 key 值，导致需要重新渲染，浪费了性能。
...
## 2 React 事件系统介绍
  ...
  * React 自己实现了一套高效的事件注册、存储、分发和重用的逻辑，在 DOM 事件体系基础上做了很大改进。不仅减少了内存消耗，最大化解决了 IE 等浏览器的不兼容问题，而且简化了事件逻辑，对开发者来说非常友好。它有如下的特点：
    ...
    * 自身实现了一套**事件冒泡 机制**，以队列形式，从触发事件的组件向父组件回溯，调用在 JSX 中绑定的 callback。因此我们也没法用 event.stopPropagation() 来停止事件传播，应该使用 React 定义的 event.preventDefault()。
...
## 4 最佳实践
  ...
  * 希望看下面代码的同学能对 ES 2017 有所了解，我们希望我们写的代码能随时跟随 ECMAScript 的步伐。所以尽可能不再使用 ES 2015 提倡的 constructor 等特性，而是使用最新的 ES 2017 类属性提案。
  ...
  * 此处感谢[@Kpax Qin](<https://www.zhihu.com/people/914db96f19a6860a1eb75cc829c02412>)对上述问题提供的解决方案，在封装好的组件外层加一层封装，这里直接上代码了。

![](https://pic2.zhimg.com/v2-f116f584ff18e928a098b4483cf2e7b9_1440w.png)

  * Rate 是阿里下蚂蚁金服 ant design 的评分组件，它自定义了 onChange 事件，默认传递参数 value，value即评分选择的星数。
  * 这里有多个 Rate 组件，它们的 value 值，我们在 state 中用一个数组进行存储。

![](https://pic3.zhimg.com/v2-64612639d566802659c0ad50974973a0_1440w.png)

  * Rate 组件不传递 event 导致当存在多个 Rate 时无法用一个 handleChange 来处理所有的 onChange 事件，只能调用 bind 来生成多个逻辑相似、传递不同参数的函数实例，这是一种内存的浪费。
  * 当外面封装了一层 div 后，基于事件冒泡机制，可以通过 event 来捕获触发事件的组件，因此就可以用一个 handleChange 函数来处理所有的 onChange 事件了。

![](https://picx.zhimg.com/v2-779a60e5872aa6288f6ebaaae6d3e087_1440w.png)![](https://pic1.zhimg.com/v2-ac0ff05078ca62255108f1c67889a710_1440w.png)

  * 但这种处理函数方式会使得业务逻辑更加复杂，因为实际上 handleChange 触发了两次，第一次触发时 value 值需要用临时变量存储，然后在第二次调用时根据临时变量和组件的 id 来对 state 中的 value 数组进行 setState 更新。
  * 如果希望更简明的业务逻辑同时对内存性能的要求不高，建议还是使用 bind(null, id) 的方式进行生成函数实例传参。类似 **4 最佳实践** 里的第一段代码。
  * 欢迎大家提出更好的解决方案。
